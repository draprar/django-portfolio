...
class TestContentManagementViews:
    ...
    def admin_user(self):
        ...
        return User.objects.create_superuser(
            username="adminuser", password="adminpassword", email="admin@example.com", is_staff=True
        )
...
class TestLoadMoreGenerics:
    ...
    def test_load_more_generic_internal_server_error(self, client, mocker, model_data):
        ...
        url = reverse(model_data[0])
        mocker.patch(
            f"tonguetwister.models.{model_data[1].__name__}.objects.all", side_effect=Exception("Test Exception")
        )
...
class TestAuthViews:
    ...
    def test_activate_success(self, client, regular_user):
        ...
        assert response.status_code == 302
        assert "Dziękujemy za potwierdzenie :) Twoje konto zostało zweryfikowane." in [
    ...
    def test_password_reset_confirm_mismatch(self, client, regular_user):
        ...
        response = client.post(
            reverse("password_reset_confirm", args=[uid, token]),
            data={"new_password1": "password123", "new_password2": "differentpassword123"},
        )
